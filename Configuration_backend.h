...
//CR-10S Model Settings
  ...
  #define DEFAULT_MAX_FEEDRATE          { 500, 500, 200, 50 } //COVID19 - Default Z value: 50
  #define DEFAULT_MAX_ACCELERATION      { 500, 500, 500, 5000 }
  ...
  // COVID19 - Based on clockwise movement of the stepper, Y moves in the negative direction (towards the stepper), and Z moves in the positive direction (away from the stepper).
  // COVID19 - We want Z's positive direction to be towards the stepper, so it does not need to be inverted. Z2 follows suit. We want Y's positive direction to be towards the stepper, so we want to invert it.
  ...
  #define INVERT_X_DIR false
    ...
    #define INVERT_Y_DIR true
  #else
    #define INVERT_Y_DIR true // COVID19 - default value: false
  #endif
  
  #if ENABLED(ENDER5_DUALBOARD)
    #define INVERT_Z_DIR false
  #else
    #define INVERT_Z_DIR false // COVID19 - default value: true
  #endif
  ...
  // COVID19 - Changed Z maximum position to match previous max values given by Kevin. This will prevent the axes from running into the build plate. 
  #if ENABLED(CR10S) || ENABLED(CRX)
    #define X_BED_SIZE 300
    #define Y_BED_SIZE 150 // COVID19 - Default value: 300
    #define Z_MAX_POS 50  // COVID19 - Default value: 400
  #endif
