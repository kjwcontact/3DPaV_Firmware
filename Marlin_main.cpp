...
//COVID19 - GLOBAL VARIABLES FOR COVID19 MOVEMENT
float covid_compress_y; //COVID19 - define covid_feedrate for M555 function
float covid_compress_z; //COVID19 - define covid_feedrate for M555 function
float covid_feedrate_1; //COVID19 - define covid_feedrate for M555 function
float covid_feedrate_2; //COVID19 - define covid_feedrate for M555 function
float covid_loopender;
...
//COVID19 - Added this function to home Y & Z at the same time
void do_blocking_move_to_yz(const float &ry, const float &rz, const float &fr_mm_s/*=0.0*/) {
  do_blocking_move_to(current_position[X_AXIS], ry, rz, fr_mm_s);
}
...
void gcode_get_destination() {
  ...
  if (parser.linearval('P') > 0) //COVID19 - load P value into Covid feedrate;
    covid_feedrate_1 = feedrate_mm_s; //COVID19 - save F feedrate into value into Covid feedrate 1;
    covid_feedrate_2 = MMM_TO_MMS(parser.value_millis());//COVID19 - load P value into Covid feedrate 2. P is usually tied into a time variable, so here, I've used parser.value_millis instead of feedrate, but I still used the MMM TO MMS Function.
...
#if ENABLED(COVID_HOME)

  static void quick_home_yz() {

    // Pretend the current position is 0,0
    current_position[Y_AXIS] = current_position[Z_AXIS] = 0.0;
    sync_plan_position();

    const float mly = max_length(Y_AXIS),
                mlz = max_length(Z_AXIS),
                mlratio = mly > mlz ? mlz / mly : mly / mlz,
                fr_mm_s = MIN(homing_feedrate(Y_AXIS), homing_feedrate(Z_AXIS)) * SQRT(sq(mlratio) + 1.0);
                //fr_mm_s = fr_mm_s / 2;
    ...
    do_blocking_move_to_yz(1.5 * mly * home_dir(Y_AXIS), 1.5 * mlz * home_dir(Z_AXIS), fr_mm_s);
...
//COVID19 - Added this in to home y and z at the same time

    #if ENABLED(COVID_HOME)

      if (home_all || (homeY && homeZ)) quick_home_yz();
...
inline void gcode_M555() {
  float covid_start_time =0;
  float covid_compress_time=0;
  float covid_end_time =0;
  float covid_decompress_time =0;
  float covid_bpm =0;
  float covid_ie =0;
  gcode_get_destination();
  covid_compress_y = destination[Y_AXIS];
  covid_compress_z = destination[Z_AXIS];
  destination[Y_AXIS] = covid_compress_y; 
  destination[Z_AXIS] = covid_compress_z;
  feedrate_mm_s = covid_feedrate_1;
  covid_loopender = 0;
  while (covid_loopender == 0) {
    prepare_move_to_destination();
    covid_start_time = millis();
    planner.synchronize();   // wait until previous movement commands (G0/G0/G2/G3) have completed
    covid_end_time = millis();
    covid_compress_time = covid_end_time - covid_start_time;
    SERIAL_PROTOCOLPAIR("COMPRESS TIME: ", covid_compress_time);
    SERIAL_PROTOCOLLNPGM("\n");
    destination[Y_AXIS] = 0;
    destination[Z_AXIS] = 0;
    feedrate_mm_s = covid_feedrate_2;
    SERIAL_PROTOCOLLNPGM(MSG_COMPRESS); //COVID19 - Say "COMPRESSDONE"
    prepare_move_to_destination();
    covid_start_time = millis();
    planner.synchronize();   // wait until previous movement commands (G0/G0/G2/G3) have completed
    covid_end_time = millis();
    covid_decompress_time = covid_end_time - covid_start_time;
    SERIAL_PROTOCOLPAIR("DECOMPRESS TIME: ", covid_decompress_time);
    SERIAL_PROTOCOLLNPGM("\n");
    covid_bpm = (60/((covid_compress_time + covid_decompress_time)/1000));
    SERIAL_PROTOCOLPAIR("BPM: ", covid_bpm);
    SERIAL_PROTOCOLLNPGM("\n");
    covid_ie = (covid_decompress_time/covid_compress_time);
    SERIAL_PROTOCOLPAIR("IE Ratio:  1 : ", covid_ie);
    SERIAL_PROTOCOLLNPGM("\n");    
    destination[Y_AXIS] = covid_compress_y;
    destination[Z_AXIS] = covid_compress_z;
    feedrate_mm_s = covid_feedrate_1;
    SERIAL_PROTOCOLLNPGM(MSG_DECOMPRESS); //COVID19 - Say "DECOMPRESSDONE"
    endstops.M119(); //COVID19 - Display Endstop States
  }
}
...
inline void gcode_M556() {
  ...
  planner.synchronize();   // wait until previous movement commands (G0/G0/G2/G3) have completed
  covid_end_time = millis();
  covid_time = covid_end_time - covid_time;
  SERIAL_PROTOCOLLN(covid_time);*/
}
...
void process_parsed_command() {
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
      ...
      //COVID19 - added in the Gcode M command for 555/556. Probably need to remove the break.
      #if ENABLED(COVID_HOME)
        case 555: gcode_M555(); break;                            // M555: COVID19 - COMPRESS COMAMND INDEX
        case 556: gcode_M556(); break;                            // M556: COVID19 - DECOMPRESS COMAMND INDEX
      #endif
